...
@onready var mariocollision = $Level_Elements/MARIO/CollisionShape2D
...
func _process(_delta: float) -> void:
	#var player = get_node(MARIO) ## This is causing console abuse. - MemeMaster
...
func intro_cutscene():
	$Level_Elements/MARIO/jump_sfx.play()
	mario.GRAVITY = 5000
...
func _on_secret_trigger_area_body_entered(body: Node2D) -> void:
	mario.can_move = false
	$UI/Transition/base/AnimationPlayer.play("appear_mario")
	
func _on_animation_player_animation_finished(anim_name: StringName) -> void:
	if anim_name == "appear_mario":
		get_tree().change_scene_to_file("res://scenes/levels/secret/faceless_area.tscn")
